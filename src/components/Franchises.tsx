...
export function Franchises({ onSelectFranchise, categories }: FranchisesProps) {
  ...
  return (
    <section id="franquicias" className="px-5 md:px-12 py-14 border-t border-outline-variant/20 bg-surface-container-lowest">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex gap-4 overflow-x-auto no-scrollbar snap-x pb-4 pt-4">
          {displayCategories.map(cat => (
            <button key={cat.id} onClick={() => handleSelect(cat.id)} className="flex-shrink-0 w-48 group p-4 rounded-xl bg-surface-container-low border border-outline-variant/30 hover:border-primary transition-all text-left space-y-3 relative overflow-hidden snap-start">
          ...
          <a href="#contacto" className="flex-shrink-0 w-48 group p-4 rounded-xl bg-surface-container border border-dashed border-primary/40 hover:border-primary transition-all text-left space-y-3 flex flex-col justify-center snap-start">
